...
| [model-effort-slider](model-effort-slider/README.md) | 基于原生模型目录的模型 / 推理档位选择器，支持自选图片、无图紧凑布局和即时节点提示 |
...
模型、供应商、effort 数量、顺序和提交值由 Harness 原生目录决定。仓库不附带图片素材；如需自定义控制头图片，请在 DSH「设置 → 推理滑块图片」中手动上传，图片只保存在当前浏览器，详见[插件说明](model-effort-slider/README.md)。
